...
(function () {
  ...
  var BRANCHES = BRANCH_DATA.map(function (d, idx) {
    var i = idx + 1;
    return { id: "br" + pad2(i), name: "지사_" + pad2(i), fee: d[0], sales: d[1], count: d[2] };
  });
  ...
  var SALESREPS = SALES_FEES.map(function (f, idx) {
    ...
    return { id: "sl" + pad2(i), name: "영업_" + pad2(i), fee: f, sales: s.sales, count: s.count,
             parentName: dist.name, grandName: dist.parentName };
  });
  var BRANDS = BRAND_FEES.map(function (f, idx) {
    ...
    return { id: "bd" + pad2(i), name: "대리점_" + pad2(i), fee: f, sales: s.sales, count: s.count,
             source: source, managerName: manager };
  });
  ...
  function getTxStatuses(owner) {
    if (!txStatusStore[owner.id]) {
      ...
      for (var i = 0; i < owner.count; i++) {
        /* _01 대상은 전건 완료 (데모: 지사_01 = 17건 / 7,000원 전액 완료) */
        arr.push(idNum === 1 ? "정산 완료" : STATUSES[(i + idNum) % STATUSES.length]);
      }
  ...
  function txAgg(owner, ownerType) {
    ...
    var agg = { total: rows.length, done: 0, need: 0, totalAmt: 0, doneAmt: 0, needAmt: 0 };
  ...
  function makeDetailRows(owner, ownerType) {
    ...
    for (var i = 1; i <= owner.count; i++) {
      ...
      feeSum += f;
      var day = 1 + ((i * 3 + owner.count) % 7);
      var hh = pad2(9 + Math.floor(rand() * 12)), mm = pad2(Math.floor(rand() * 60));
      rows.push({
        seller: ownerType === "대리점" ? owner.name : "대리점_" + pad2((i % 4) + 1),
        product: PRODUCTS[i % PRODUCTS.length],
        orderNo: "2026070" + day + String(1000000 + i * 17 + Number(owner.id.replace(/\D/g, "")) * 131),
        soldAt: "2026-07-0" + day + " " + hh + ":" + mm,
        buyer: i % 2 ? "김링크" : "이허브",
        payMethod: i % 2 ? "카드" : "간편결제",
        sales: amt, rate: owner.fee, fee: f, settle: amt - f
      });
    }
    return rows;
  }
  ...
  function initDetail() {
    ...
    function ownerCard() {
      ...
      if (isBrand) {
        ...
        retHtml = "<div class='kv'><b>유보금</b> " + num(rst.balance) + "원 / 한도 " + num(rst.conf.limit) + "원 (" + rst.conf.rate + "%, 결제 원금 기준)" +
      ...
      byId("ownerCard").innerHTML =
        ...
        "<div class='kv'><b>총 판매금액</b> " + num(owner.sales) + "원</div>" +
        "<div class='kv'><b>지불해야 할 정산 금액</b> <b style='color:#166534'>" + num(owner.sales - fee(owner.sales, owner.fee)) + "원</b></div>" + retHtml;
    }
    ...
    function render() {
      /* KPI 6종 — 현재 필터된 거래 기준 */
      var agg = { total: filtered.length, done: 0, need: 0, totalAmt: 0, doneAmt: 0, needAmt: 0 };
      filtered.forEach(function (x) {
        agg.totalAmt += x.row.settle;
        if (sts[x.idx] === "정산 완료") { agg.done++; agg.doneAmt += x.row.settle; }
        else { agg.need++; agg.needAmt += x.row.settle; }
      });
      byId("settlementKpiGrid").innerHTML = kpi6(agg);
      byId("salesTableBody").innerHTML = filtered.map(function (x) {
        var r = x.row, i = x.idx;
        return "<tr><td class='text-center'><input type='checkbox' class='tx-check' data-i='" + i + "' " + (selected[i] ? "checked" : "") + " /></td>" +
          ...
          "<td class='text-center'>" + statusBadge(sts[i]) + "</td>" +
    ...
    renderRunPanel(byId("cyclePanel"), function (c) { run = c; render(); });
    byId("runBase").value = base;
    byId("runRange").textContent = base + " 정산 런";
    applyFilter();

    byId("searchBtn").addEventListener("click", applyFilter);
    byId("resetBtn").addEventListener("click", function () { byId("fStatus").value = ""; applyFilter(); });
    byId("selectAllBtn").addEventListener("click", function () {
      filtered.forEach(function (x) { selected[x.idx] = true; });
      render();
    });
    ...
    byId("downloadBtn").addEventListener("click", function () {
      A.downloadCSV(owner.name + "_정산상세_" + run.date + ".csv",
        ["판매처", "판매 상품", "주문번호", "판매 일시", "판매금액", "수수료율", "수수료", "지불해야 할 정산 금액", "정산 상태"],
        filtered.map(function (x) { return [x.row.seller, x.row.product, x.row.orderNo, x.row.soldAt, x.row.sales, x.row.rate + "%", x.row.fee, x.row.settle, sts[x.idx]]; }));
    });
    byId("salesTableBody").addEventListener("click", function (e) {
      ...
      var r = detailRows[i];
      openModal("거래 상세 — " + r.orderNo,
        infoRows([
          ["판매처", r.seller], ["상품명", r.product], ["주문번호", r.orderNo],
          ["구매자", r.buyer], ["결제수단", r.payMethod], ["판매일시", r.soldAt],
          ["판매금액", money(r.sales)], ["적용 수수료율", r.rate + "%"],
          ["수수료", money(r.fee)], ["지불해야 할 정산 금액", money(r.settle)],
          ["정산 상태", statusBadge(sts[i])]
        ]));
    });
  }
